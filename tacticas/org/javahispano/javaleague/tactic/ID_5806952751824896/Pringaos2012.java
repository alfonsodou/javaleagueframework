package org.javahispano.javaleague.tactic.ID_5806952751824896;
...
public class Pringaos2012 implements Tactic {
	...
	public List<Command> execute(GameSituations sp) {
		...
		for (int i = -1; i < jugadores.length; i++) {
			if (sp.iteration() == 0) {
				...
				int count;
				double distance = jugadores[i].distance(sp.rivalPlayers()[nearest]);
				if (distance < Constants.JUGADORES_SEPARACION) {
					nueva = jugadores[i];
					count = 0;
					while ((distance < Constants.JUGADORES_SEPARACION) && (count < 20)) {
						angulo = r.nextInt(360) * Math.PI / 180;
						nueva = jugadores[i].moveAngle(angulo, Constants.getVelocidad(sp.getMyPlayerSpeed(i)));
						if (nueva.isInsideGameField(0)) {
							distance = nueva.distance(sp.rivalPlayers()[nearest]);
						} else {
							distance = 0;
						}
						count++;
		...
		for (int i : sp.canKick()) {
			...
			if (i == 8 || i == 10) {
				...
				for(int c = 0; c < pases.size(); c++) {
					...
					pases.get(c).addScore((double)(1.0 / pases.get(c).getIteraciones()));
				...
				comandos.add(new CommandHitBall(i, 
												jugadores[pases.get(max_index).getId_player_target()], 
												pases.get(max_index).getPower(),
												pases.get(max_index).getAngle()));				
			}
		}
        ...
        while ((!found)&&(it < 100)) {
            ...
            if (posBalon[2] <= Constants.ALTO_ARCO) {
                for (i = 0; i < sp.myPlayers().length; i++) {
                    if (posBalon[2] <= (sp.myPlayersDetail()[i].isGoalKeeper() ? Constants.ALTO_ARCO : Constants.ALTURA_CONTROL_BALON)) {
                        pJug = sp.myPlayers()[i];
                        dist0 = (double) it * Constants.getVelocidad(sp.getMyPlayerSpeed(i));
        ...
        while ((!found)&&(it < 100)) {
            ...
            if (posBalon[2] <= Constants.ALTO_ARCO) {
                for (i = 0; i < sp.rivalPlayers().length; i++) {
                    if (posBalon[2] <= (sp.rivalPlayersDetail()[i].isGoalKeeper() ? Constants.ALTO_ARCO : Constants.ALTURA_CONTROL_BALON)) {
                        pJug = sp.rivalPlayers()[i];
                        dist0 = (double) it * Constants.getVelocidad(sp.getRivalPlayerSpeed(i));
                        dist = pJug.distance(new Position(posBalon[0], posBalon[1]));
                        if (dist0 >= dist) {
                            found = true;
                            idxFound = it;
    ...
    public LinkedList<Pases> isPassSafeFromOpponent(GameSituations sp, int player) {
    	double angulo;
    	double fuerza;
    	double fuerzaTotal;
    	double anguloH;
        ...
        for(angulo = 0; angulo <= Constants.ANGULO_VERTICAL_MAX; angulo += 10) {
        	...
        	for(fuerza = 0.2f; fuerza <= 1; fuerza += 0.2f) {
                ...
                for(anguloH = 0.0f; anguloH < 360; anguloH += 10) {
                	tra = new Trayectoria(p1, Constants.getVelocidadRemate(fuerzaTotal), (anguloH * Math.PI) / 180, angulorad);
                	double distance2 = p1.distance(new Position(tra.getPos(99)[0], tra.getPos(99)[1]));
                	...
                	if ((iteraciones != null) &&
                		...
                		if ((iteraciones[0] < iteracionesrival) || ((iteracionesrival == -1) && (iteraciones[0] != -1))) {

                			pases.add(new Pases(new Position(tra.getPos(iteraciones[0])[0], tra.getPos(iteraciones[0])[1]), 
                									  0, 
                									  angulo, 
                									  fuerza,
                									  iteraciones[0],
...
class Trayectoria {
	...
	public Trayectoria(Position pos, double vel, double ang, double angV) {
		...
		AbstractTrajectory t = new AirTrajectory(vel * Math.cos(angV), 
												 vel * Math.sin(angV), 
												 0, 
												 0);
		for (int i = 0; i < 100; i++) {
			radio = t.getX((double)(i + 1) / 60d) * Constants.AMPLIFICA_VEL_TRAYECTORIA;
			positions[i][0] = pos.getX() + radio * Math.cos(ang); 
			positions[i][1] = pos.getY() + radio * Math.sin(ang);
			positions[i][2] = t.getY((double)(i + 1)/60d) * Constants.AMPLIFICA_VEL_TRAYECTORIA * 2;
		}
	}
...
class Pases {
	...
	public Pases(Position p, double s, double a, double po, int it, int source, int target) {
		pos = p;
		score = s;
		angle = a;
		power = po;
