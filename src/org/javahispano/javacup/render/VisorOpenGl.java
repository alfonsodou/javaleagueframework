	...
	public void update(GameContainer gc, int fps) throws SlickException {
		...
		if (guardado && progreso) {
			if (i.isMouseButtonDown(0)) {
				double y = 1
						- (double) (Math.max(20,
								Math.min(i.getMouseY(), sy - 20)) - 20)
						/ (double) (sy - 40);
				PartidoGuardado pguardado = (PartidoGuardado) partido;
				pguardado.setTiempo((int) (y * (double) pguardado
						.getIterciones()));
			}
		}
	...
	public void render(GameContainer gc, Graphics g) throws SlickException {
		try {
			...
			if (autoescala) {
				...
				if (!noAutoEscalar) {
					...
					if (partido.getIteracion() < 50) {
						escalaGradual = escalaGradual
								+ (escala - escalaGradual) * 0.01;
					} else {
						escalaGradual = escalaGradual
								+ (escala - escalaGradual) * 0.2;
					}
				}
			...
			if (!partido.esGol() && partido.estanSacando()) {
				...
				rel = Transforma.transform(pos[2][0],
						Constants.centroCampoJuego,
						-Transforma.transform(px, escala) + sx2,
						-Transforma.transform(py, escala) + sy2, escala);
				g.drawImage(xImage.getScaledCopy((int) zoom, (int) zoom),
						rel[0] - (int) (zoom / 2), rel[1] - (int) (zoom / 2));
			...
			for (int i = 0; i < 11; i++) {
				rel = Transforma.transform(pos[0][i],
						...
						-Transforma.transform(px, escala) + sx2,
						-Transforma.transform(py, escala) + sy2, escala);
				pjLocal.pintaSombra(i, iteraciones[i][0], angVisible[i][0],
			...
			z = partido.getAlturaBalon();// 16*Math.sin(Math.abs(ang %
											// Math.PI));
			...
			if (partido.getAlturaBalon() <= 2) {
				pb.pintaBalon((int) (giro), ang, escala, rel[0], rel[1], z * 2,
						g);
			}
			...
			if (jugador3d) {
				...
				for (int i = 0; i < 11; i++) {
					rel = Transforma.transform(pos[0][i],
							...
							-Transforma.transform(px, escala) + sx2,
							-Transforma.transform(py, escala) + sy2, escala);
					pjLocal.pintaJugador(i, iteraciones[i][0],
							angVisible[i][0], escala, rel[0], rel[1], g);
					if (tipoTexto == 3
							|| (pos[0][i].distance(pos[2][0]) < 8 && tipoTexto == 1)) {
						pjLocal.pintaNumero(i, rel[0], rel[1], g);
					}
					...
					rel = Transforma.transform(pos[1][i],
							...
							-Transforma.transform(px, escala) + sx2,
							-Transforma.transform(py, escala) + sy2, escala);
					pjVisita.pintaJugador(i, iteraciones[i][1],
							angVisible[i][1], escala, rel[0], rel[1], g);
					if (tipoTexto == 3
							|| (pos[1][i].distance(pos[2][0]) < 8 && tipoTexto == 1)) {
						pjVisita.pintaNumero(i, rel[0], rel[1], g);
					}
			...
			if (marcador) {
				pm.pintaMarcador(partido.getGolesLocal(),
						partido.getGolesVisita(), partido.getIteracion(),
						partido.getPosesionBalonLocal(), g);
			}
			if (golIter > 0) {
			...
			if (guardado && progreso) {
				...
				int valor = (int) (((double) sy - 41.0) * ((double) pg
						.getTiempo() / (double) pg.getIterciones()));
				...
				g.setColor(Color.white);
				g.drawLine(sx - 19, sy - 22 - valorFin, sx - 12, sy - 22
						- valorFin);
				g.drawLine(sx - 19, sy - 21 - valorFin, sx - 18, sy - 21
						- valorFin);
				g.drawLine(sx - 13, sy - 21 - valorFin, sx - 12, sy - 21
						- valorFin);
			...
			if (showTexto) {
				if (partido.getIteracion() > Constants.ITERACIONES) {
					...
					if ((partido.getGolesLocal() > partido.getGolesVisita())
							|| (partido.getGolesLocal() == partido
									.getGolesVisita() && partido
			 ...
			 * rand.nextDouble() * 1024; int x0 = (int) (sx2 + Math.sin(an) *
			 * rad); int y0 = (int) (sy2 + Math.cos(an) * rad); int x1 = (int)
			 * (sx2 + Math.sin(an) * rad * 1.1d); int y1 = (int) (sy2 +
			 * Math.cos(an) * rad * 1.1d); g.drawLine(x0, y0, x1, y1); } if
			 * (rand.nextDouble() < 0.05) { g.setColor(relampago); g.fillRect(0,
			 * 0, sx, sy); } isRain = rand.nextDouble() < 0.995; } else { isRain
